...
class LossHistory():
    ...
    def loss_plot(self):
        ...
        try:
            ...
            if sp_signal is not None and len(self.losses) >= num and len(self.val_loss) >= num:
                smooth_train = sp_signal.savgol_filter(self.losses, num, 3)
                smooth_val = sp_signal.savgol_filter(self.val_loss, num, 3)
            else:
                # NumPy fallback: simple moving average with 'same' padding
                kernel = np.ones(num) / num
                smooth_train = np.convolve(self.losses, kernel, mode='same')
                smooth_val = np.convolve(self.val_loss, kernel, mode='same')
