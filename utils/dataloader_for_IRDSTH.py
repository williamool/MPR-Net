"""
Sequence data loader for IRDST-H.

Annotation txt format (one frame per line):
    <path>/<sequence>/<frame_id>.bmp x1,y1,x2,y2,cls [x1,y1,x2,y2,cls ...]
Images are letterbox-resized to the configured input size (grey padding 128); motion difference
maps use the same geometry with zero padding.
"""
import os
        ...
        for id in range(self.num_frame):
            ...
            if not os.path.exists(motion_diff_path):
                ...
                if motion_diff is None:
                    ...
                    new_motion_diff = np.full((h, w), 0, dtype=np.uint8)
