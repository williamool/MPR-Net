"""
MPR-Net for IRSTD-UAV.

Feature_Extractor  : CSPDarknet with the Motion-Guided Feature Enhancement (MFE) module + PAFPN neck -> F_f
TargetTextEncoder  : Motion Prototype Initialisation (frozen CLIP ViT-B/32 text encoder + learnable context tokens)
ScoreCompute       : prototype-guided score map + prototype-enhanced feature (QKV-style attention, LogSumExp)
GraphUpdate        : two-layer GCN for Graph-Based Feature Propagation over Top-K candidates
CleanFusionModule  : joint modelling of F_f and the graph-refined F_r (Eq. 31-32)
YOLOXHead          : decoupled detection head (single stride-8 level)

Identical architecture to nets/MPR_DAUB.py / nets/MPR_IRDSTH.py; only the coarse textual
...
class MPR(nn.Module):
    ...
    def __init__(self, num_classes, num_frame=2, text_input_dim=20 * 300):
        ...
        self.target_classes = [
            "a small bright moving target in infrared UAV imagery",
            "a tiny hot flying object in thermal aerial background",
            "a small moving target in infrared UAV scene",
            "a weak small target with low contrast in IR video",
        ]
