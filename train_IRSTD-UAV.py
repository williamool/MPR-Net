"""
Train MPR-Net on IRSTD-UAV.

    python train_IRSTD-UAV.py \
        --train_txt  datasets/IRSTD-UAV/train.txt \
        --val_txt    datasets/IRSTD-UAV/val.txt \
        ...
        --save_dir   logs/IRSTD-UAV

All hyper-parameters default to the setting used in the paper
(512x640 (HxW) letterbox input, 100 epochs, batch 8, SGD lr 1e-2, momentum 0.937, wd 5e-4, cosine decay).
"""
import argparse
import datetime
import os
...
from utils.dataloader_for_IRSTD_UAV import dataset_collate, seqDataset
from utils.utils import get_classes, show_config
from utils.utils_fit import fit_one_epoch


def parse_args():
    p = argparse.ArgumentParser(description="Train MPR-Net on IRSTD-UAV")
    # ---------------- data ----------------
    p.add_argument("--train_txt", default="datasets/IRSTD-UAV/train.txt")
    p.add_argument("--val_txt", default="datasets/IRSTD-UAV/val.txt")
    p.add_argument("--images_root", default="datasets/IRSTD-UAV/images",
                   help="image root containing the sequence folders 1/, 2/, ... (used to re-base the paths stored in the txt)")
    ...
    p.add_argument("--input_shape", type=int, nargs=2, default=[512, 640], metavar=("H", "W"),
                   help="IRSTD-UAV frames are 640x512 (WxH); the default keeps the native aspect ratio")
    # ---------------- optimisation ----------------
    ...
    # ---------------- logging ----------------
    p.add_argument("--save_dir", default="logs/IRSTD-UAV")
...
if __name__ == "__main__":
    ...
    # evaluate the saved checkpoints with test_IRSTD-UAV.py instead.
