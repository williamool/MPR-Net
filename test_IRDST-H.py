"""
Evaluate MPR-Net on the IRDST-H test set (COCO protocol: mAP50 / Precision / Recall / F1).

    python test_IRDST-H.py \
        --weights     logs/IRDST-H_best.pth \
        --coco_json   datasets/IRDST-H/test.json \
        --images_root datasets/IRDST-H/images \
        --motion_root datasets/motion_difference_map_IRDST-H

Inference follows the paper: 512x512 letterbox, confidence >= 0.001, NMS IoU 0.65.
"""
import argparse
import json
import os
...
from nets.MPR_IRDSTH import MPR
from utils.dataloader_for_IRDSTH import preprocess_motion_diff
from utils.utils import cvtColor, get_classes, preprocess_input, resize_image, show_config
from utils.utils_bbox import decode_outputs, non_max_suppression

# value used to pad the letterboxed motion difference map (must match the training loader; 0 for IRDST-H)
MOTION_PAD_VALUE = 0
NUM_FRAME = 2  # frames t-1 and t are fed to the network; only frame t is used by the detector


def parse_args():
    p = argparse.ArgumentParser(description="Evaluate MPR-Net on IRDST-H")
    p.add_argument("--weights", default="logs/IRDST-H_best.pth")
    p.add_argument("--coco_json", default="datasets/IRDST-H/test.json", help="COCO-format ground truth")
    p.add_argument("--images_root", default="datasets/IRDST-H/images", help="root joined with the COCO file_name")
    p.add_argument("--motion_root", default="datasets/motion_difference_map_IRDST-H",
    ...
    p.add_argument("--out_dir", default="results/IRDST-H")
...
class Detector(object):
    ...
    def load_motion_diff_data(self, image_path):
        ...
        for k in range(NUM_FRAME):
            ...
            motion_diff = cv2.imread(motion_diff_path, cv2.IMREAD_GRAYSCALE) if os.path.exists(motion_diff_path) else None
...
def get_history_imgs(path):
    """<dir>/<id>.bmp -> [<dir>/<id-1>.bmp, <dir>/<id>.bmp]"""
    dir_path = os.path.dirname(path)
    stem, ext = os.path.splitext(os.path.basename(path))
    index = int(stem)
    return [os.path.join(dir_path, f"{max(i, 0)}{ext}") for i in range(index - NUM_FRAME + 1, index + 1)]
