"""
Evaluate MPR-Net on the DAUB-R test set (COCO protocol: mAP50 / Precision / Recall / F1).

    python test_DAUB.py \
        --weights     logs/DAUB_best.pth \
        --coco_json   datasets/DAUB/test.json \
...
from utils.dataloader_for_DAUB import preprocess_motion_diff
...
def parse_args():
    p = argparse.ArgumentParser(description="Evaluate MPR-Net on DAUB-R")
    p.add_argument("--weights", default="logs/DAUB_best.pth")
    p.add_argument("--coco_json", default="datasets/DAUB/test.json", help="COCO-format ground truth")
    p.add_argument("--images_root", default="datasets/DAUB", help="root joined with the COCO file_name")
    ...
    p.add_argument("--out_dir", default="results/DAUB")
